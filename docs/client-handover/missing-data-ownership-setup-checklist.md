...
| Stock at cost and aging | Existing lot/ledger workflow; legacy zero-cost lots are actionable exceptions | Inventory manager | Receive or open stock with approved unit cost and maintain lot movement chronology. For a legacy `LOT COST MISSING` exception, expand the inventory lot and record the verified unit cost plus source/reason. The guarded action only fills a non-positive cost and writes audit provenance; it cannot silently overwrite an established cost. |
...
| Password-reset delivery | Token lifecycle and secure operator-issued link exist; automatic delivery provider is not configured | Administrator / support | For an immediate lockout, issue the one-hour link with the production operator script and hand it privately to the named user. For self-service recovery, configure an approved mail provider/domain and verify one-time delivery, expiry, and audit without exposing the token. |
