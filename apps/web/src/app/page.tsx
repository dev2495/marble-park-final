...
export default function HomePage() {
  return (
    <main className="min-h-screen overflow-hidden bg-[#f4f7fc] text-[#18181b]">
      <section className="relative min-h-screen px-6 py-6 lg:px-10">
        ...
        <div className="absolute inset-x-8 bottom-0 h-52 rounded-t-[4rem] bg-[#18181b]/10 blur-3xl" />
        ...
        <div className="relative mx-auto flex min-h-[calc(100vh-3rem)] max-w-7xl flex-col rounded-[2.5rem] border border-white/55 bg-white/55 p-5 shadow-2xl shadow-[#475569]/20 backdrop-blur-2xl lg:p-8">
          <header className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <div className="grid h-12 w-12 place-items-center rounded-2xl bg-[#18181b] text-xl font-black text-[#f4f7fc] shadow-xl">MP</div>
              <div>
                <div className="text-lg font-black leading-none tracking-tight">Marble Park</div>
                <div className="mt-1 text-xs font-medium uppercase tracking-[0.14em] text-[#52525b]">Retail OS</div>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <Link href="/login" className="rounded-2xl border border-[#0e1a3d]/10 bg-white/70 px-5 py-3 text-sm font-semibold text-[#18181b] shadow-sm transition hover:bg-white">Login</Link>
              <Link href="/dashboard" className="hidden rounded-2xl bg-[#18181b] px-5 py-3 text-sm font-black text-white shadow-xl transition hover:translate-y-[-1px] sm:inline-flex">Open App</Link>
            </div>
          </header>

          <div className="grid flex-1 items-center gap-10 py-12 lg:grid-cols-[1fr_0.86fr] lg:py-6">
            <div className="max-w-3xl">
              <h1 className="text-6xl font-black leading-[0.88] tracking-[-0.06em] text-[#18181b] sm:text-7xl lg:text-8xl">
              ...
              <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                ...
                <Link href="/dashboard/products" className="inline-flex h-14 items-center justify-center rounded-2xl border border-[#0e1a3d]/10 bg-white/70 px-7 text-sm font-black uppercase tracking-wider text-[#18181b] shadow-sm transition hover:bg-white">
                  View catalogue
                </Link>
              </div>
            </div>

            <div className="relative min-h-[520px] overflow-hidden rounded-r6 bg-[#18181b] p-5 text-white shadow-2xl shadow-[#0e1a3d]/25">
              ...
              <div className="relative grid h-full grid-rows-[1fr_auto] gap-5">
                <div className="grid grid-cols-2 gap-4">
                  {features.map(([title, body, Icon], index) => (
                    <div key={String(title)} className={`rounded-r4 border border-white/10 bg-white/[0.08] p-5 backdrop-blur ${index === 0 ? 'col-span-2' : ''}`}>
                      <Icon className="mb-5 h-9 w-9 text-[#71717a]" strokeWidth={1.4} />
