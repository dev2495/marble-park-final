  ...
  return (
    <div className="space-y-7 pb-10">
      ...
      <section className="grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="relative overflow-hidden rounded-[2.25rem] bg-[#241b14] p-8 text-white shadow-2xl shadow-[#241b14]/18">
          ...
          <div className="relative max-w-3xl">
            <div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/10 px-4 py-2 text-[10px] font-black uppercase tracking-[0.24em] text-[#ead7bd]"><ShieldCheck className="h-4 w-4" /> {effectiveRole.replace('_', ' ')} landing</div>
            <h1 className="mt-4 text-5xl font-black leading-[0.92] tracking-[-0.055em] lg:text-7xl">{copy.title}</h1>
            <p className="mt-5 max-w-2xl text-base font-semibold leading-7 text-[#f6eadb]">{copy.body}</p>
            <div className="mt-7 flex flex-wrap gap-3">
              <Button asChild size="lg" className="bg-[#ffffff] text-[#241b14] hover:bg-white"><a href={copy.href}>{copy.cta}</a></Button>
          ...
          {roleCards.map((card: any, index: number) => (
            <motion.div key={card.label} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} className="mp-card rounded-[2rem] p-5">
              <div className="flex items-center justify-between">
                <div className="grid h-12 w-12 place-items-center rounded-2xl bg-[#f6eadb] text-[#8a552e]"><ArrowUpRight className="h-6 w-6" strokeWidth={1.5} /></div>
                <span className="text-[10px] font-black uppercase tracking-widest text-[#6f6258]">{card.note || 'live'}</span>
              </div>
              <div className="mt-5 text-4xl font-black tracking-[-0.04em] text-[#241b14]">{loading ? '...' : card.value}</div>
              <div className="mt-1 text-sm font-black text-[#6f6258]">{card.label}</div>
            </motion.div>
          ))}
        </div>
      </section>
          ...
          <div className="mp-card rounded-[2rem] p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-sm font-semibold text-[#6f6258]">Leads, quote value and won value by user.</p>
              </div>
              <Users className="h-7 w-7 text-[#047857]" />
            </div>
            <div className="mt-5 overflow-hidden rounded-[1.5rem] border border-[#d9cbbd]/10">
              <table className="w-full text-left text-sm">
                <thead className="bg-[#f6eadb]/70 text-[10px] uppercase tracking-widest text-[#6f6258]"><tr><th className="p-3">User</th><th className="p-3">Leads</th><th className="p-3">Quotes</th><th className="p-3">Won</th><th className="p-3 text-right">Value</th></tr></thead>
                <tbody className="divide-y divide-[#d9cbbd]/10 bg-white/50">
                  {userPerformance.map((row: any) => (
                    <tr key={row.id}>
                      <td className="p-3"><div className="font-black text-[#241b14]">{row.name}</div><div className="text-xs font-bold text-[#6f6258]">{row.role}</div></td>
      ...
      {effectiveRole === 'dispatch_ops' && <section className="mp-card rounded-[2rem] p-6"><h2 className="text-2xl font-black">Pending dispatch jobs</h2><div className="mt-5 grid gap-3 lg:grid-cols-2">{dispatchJobs.slice(0, 8).map((job: any) => <div key={job.id} className="rounded-2xl bg-white/65 p-4"><div className="font-black">{job.customer?.name || job.customer?.companyName || 'Customer'}</div><div className="mt-1 text-sm font-bold text-[#6f6258]">{job.siteAddress}</div><div className="mt-3 text-xs font-black uppercase tracking-widest text-[#b17643]">{job.status}</div></div>)}</div></section>}
    </div>
  );
}
