        ...
        <div className="divide-y divide-[#e4e4e7]">
          ...
          {quotes.map((quote: any) => {
            ...
            return <article key={quote.id} className="grid gap-4 p-5 transition hover:bg-[#fffaf9] lg:grid-cols-[1.25fr_1fr_0.9fr_0.72fr_0.85fr_0.72fr_1fr] lg:items-center">
              ...
              <div className="flex flex-wrap gap-2 lg:justify-end"><Button asChild variant="outline" size="sm"><Link href={`/dashboard/quotes/${quote.id}`}><Eye className="mr-1 h-4 w-4"/>View</Link></Button><Button asChild variant="outline" size="sm"><a href={`/api/pdf/quote/${quote.id}`} target="_blank" rel="noreferrer"><Download className="mr-1 h-4 w-4"/>PDF</a></Button>{!['sent', 'confirmed', 'won', 'closed'].includes(quote.status) ? <Button disabled={sending || !commercial.pricingReady} onClick={() => sendQuote({ variables: { id: quote.id } })} variant="warning" size="sm"><Send className="mr-1 h-4 w-4"/>Send</Button> : null}</div>
            </article>;
          })}
        </div>
      </section>
