  ...
  return <div className="space-y-6 pb-10">
    ...
    <section className="relative overflow-hidden rounded-[2.25rem] bg-[#211b16] p-7 text-white shadow-2xl shadow-[#211b16]/15">
      ...
      <div className="relative flex flex-col justify-between gap-6 xl:flex-row xl:items-end">
        <div>
          ...
          <p className="mt-6 text-[10px] font-black uppercase tracking-[0.28em] text-[#e8c39b]">{quote.status} · {quote.approvalStatus} · {displayMode === 'selection' ? 'selection summary' : 'priced quote'}</p>
        ...
        <div className="flex flex-wrap gap-3">
          <Button disabled={savingQuote} onClick={saveQuote} size="lg" className="bg-[#e8c39b] text-[#211b16] hover:bg-[#f0d2ad]"><Save className="mr-2 h-5 w-5" /> Save quote layout</Button>
    ...
    <section className="grid gap-5 xl:grid-cols-[1fr_0.42fr]">
      <div className="space-y-5">
        <div className="mp-card rounded-[2rem] p-5">
          <div className="grid gap-4 md:grid-cols-4">
            <label className="space-y-2"><span className="text-[10px] font-black uppercase tracking-wider text-[#8b6b4c]">PDF type</span><select value={displayMode} onChange={(event)=>setDisplayMode(event.target.value)} className="h-11 w-full rounded-2xl border border-[#7a5b3c]/15 bg-white px-4 text-sm font-black"><option value="priced">Show prices - quotation</option><option value="selection">Hide prices - selection summary</option></select></label>
            <label className="space-y-2"><span className="text-[10px] font-black uppercase tracking-wider text-[#8b6b4c]">Quote discount %</span><input type="number" value={discountPercent} onChange={(event)=>setDiscountPercent(event.target.value)} className="h-11 w-full rounded-2xl border border-[#7a5b3c]/15 bg-white px-4 text-sm font-black" /></label>
            <label className="space-y-2 md:col-span-2"><span className="text-[10px] font-black uppercase tracking-wider text-[#8b6b4c]">Remarks</span><input value={remarks} onChange={(event)=>setRemarks(event.target.value)} className="h-11 w-full rounded-2xl border border-[#7a5b3c]/15 bg-white px-4 text-sm font-black" /></label>
          </div>
        </div>

        {grouped.map((group) => <div key={group.area} className="overflow-hidden rounded-[2rem] border border-[#7a5b3c]/10 bg-[#fffaf3]/85 shadow-xl shadow-[#6b4f38]/8">
          <div className="border-b border-[#7a5b3c]/10 bg-[#211b16] px-5 py-4 text-[10px] font-black uppercase tracking-widest text-[#e8c39b]">{group.area} · {group.rows.length} item(s)</div>
            ...
            {group.rows.map((line: any) => {
              ...
              return <article key={`${line.sku}-${index}`} className="grid gap-4 p-5 xl:grid-cols-[8rem_1fr_7rem_7rem_7rem_7rem] xl:items-center">
                ...
                <div className="space-y-2">
                  <input value={line.area || ''} onChange={(event)=>updateLine(index,{area:event.target.value})} className="h-9 w-full rounded-xl border border-[#7a5b3c]/15 bg-white px-3 text-xs font-black uppercase tracking-wider text-[#b57942]" placeholder="Area / room" />
                  <p className="text-lg font-black text-[#211b16]">{line.name}</p>
                  <p className="text-xs font-black uppercase tracking-wider text-[#8b6b4c]">{line.sku || line.tileCode} · {line.brand || line.category || ''}</p>
                  <div className="flex items-center gap-2"><ImagePlus className="h-4 w-4 text-[#b57942]"/><input value={line.quoteImage || ''} onChange={(event)=>updateLine(index,{quoteImage:event.target.value})} placeholder="Optional quote image URL" className="h-9 flex-1 rounded-xl border border-[#7a5b3c]/15 bg-white px-3 text-xs font-bold" /></div>
                </div>
                <label className="space-y-1"><span className="text-[10px] font-black uppercase tracking-wider text-[#8b6b4c]">Qty</span><input type="number" value={line.qty || line.quantity || 0} onChange={(event)=>updateLine(index,{qty:Number(event.target.value)})} className="h-10 w-full rounded-xl border border-[#7a5b3c]/15 bg-white px-3 text-sm font-black" /></label>
                <label className="space-y-1"><span className="text-[10px] font-black uppercase tracking-wider text-[#8b6b4c]">MRP</span><input type="number" value={line.price || line.sellPrice || 0} onChange={(event)=>updateLine(index,{price:Number(event.target.value),sellPrice:Number(event.target.value)})} className="h-10 w-full rounded-xl border border-[#7a5b3c]/15 bg-white px-3 text-sm font-black" /></label>
                <label className="space-y-1"><span className="text-[10px] font-black uppercase tracking-wider text-[#8b6b4c]">Disc %</span><input type="number" value={line.discountPercent || line.discount || 0} onChange={(event)=>updateLine(index,{discountPercent:Number(event.target.value)})} className="h-10 w-full rounded-xl border border-[#7a5b3c]/15 bg-white px-3 text-sm font-black" /></label>
                <div className="text-right"><p className="text-[10px] font-black uppercase tracking-wider text-[#8b6b4c]">Special / total</p><p className="mt-1 font-black text-[#211b16]">{money(rate.specialRate)}</p>{showPrices && <p className="text-xl font-black text-[#24544d]">{money(rate.amount)}</p>}</div>
              </article>;
            })}
          </div>
        </div>)}
      ...
      <aside className="space-y-5">
        <div className="mp-card rounded-[2rem] p-6"><h2 className="text-2xl font-black tracking-tight">Customer</h2><p className="mt-4 text-lg font-black text-[#211b16]">{quote.customer?.name || 'Customer'}</p><p className="mt-2 text-sm font-bold text-[#7d6b5c]">{quote.customer?.mobile || quote.customer?.phone}</p><p className="mt-2 text-sm font-bold text-[#7d6b5c]">{quote.customer?.siteAddress || quote.customer?.city}</p></div>
        <div className="mp-card rounded-[2rem] p-6"><h2 className="text-2xl font-black tracking-tight">Totals</h2>{showPrices ? <div className="mt-5 space-y-3 text-sm font-bold text-[#5f4b3b]"><div className="flex justify-between"><span>Subtotal</span><span>{money(subtotal)}</span></div><div className="flex justify-between"><span>Discount</span><span>{money(quoteDiscount)}</span></div><div className="flex justify-between"><span>GST 18%</span><span>{money(tax)}</span></div><div className="flex justify-between border-t border-[#7a5b3c]/10 pt-4 text-2xl font-black text-[#211b16]"><span>Total</span><span>{money(total)}</span></div></div> : <p className="mt-4 rounded-2xl bg-[#ead7c0]/70 p-4 text-sm font-black text-[#7a4f2e]">Selection summary mode hides all prices in the PDF.</p>}</div>
        <div className="mp-card rounded-[2rem] p-6"><h2 className="text-2xl font-black tracking-tight">PDF terms</h2><label className="mt-4 block space-y-2"><span className="text-[10px] font-black uppercase tracking-wider text-[#8b6b4c]">Terms</span><textarea value={terms} onChange={(event)=>setTerms(event.target.value)} className="min-h-28 w-full rounded-2xl border border-[#7a5b3c]/15 bg-white px-4 py-3 text-xs font-bold" /></label><label className="mt-3 block space-y-2"><span className="text-[10px] font-black uppercase tracking-wider text-[#8b6b4c]">Bank details</span><textarea value={bankDetails} onChange={(event)=>setBankDetails(event.target.value)} className="min-h-24 w-full rounded-2xl border border-[#7a5b3c]/15 bg-white px-4 py-3 text-xs font-bold" /></label></div>
        <div className="mp-card rounded-[2rem] p-6"><h2 className="text-2xl font-black tracking-tight">Convert to sales order</h2><p className="mt-2 text-sm font-bold text-[#8b6b4c]">Use after final customer confirmation. Cash orders capture advance/full payment; credit orders are tagged for owner reports.</p><label className="mt-4 block space-y-2"><span className="text-[10px] font-black uppercase tracking-wider text-[#8b6b4c]">Payment</span><select value={paymentMode} onChange={(e)=>setPaymentMode(e.target.value)} className="h-11 w-full rounded-2xl border border-[#7a5b3c]/15 bg-white px-4 text-sm font-black"><option value="cash">Cash</option><option value="credit">Credit</option></select></label>{paymentMode === 'cash' && <label className="mt-3 block space-y-2"><span className="text-[10px] font-black uppercase tracking-wider text-[#8b6b4c]">Advance / full paid</span><input type="number" value={advanceAmount} onChange={(e)=>setAdvanceAmount(e.target.value)} className="h-11 w-full rounded-2xl border border-[#7a5b3c]/15 bg-white px-4 text-sm font-black" /></label>}{orderMessage && <p className="mt-3 rounded-2xl bg-[#ead7c0]/70 p-3 text-xs font-black uppercase tracking-wider text-[#7a4f2e]">{orderMessage}</p>}<Button className="mt-4 w-full" disabled={creatingOrder || quote.approvalStatus === 'pending'} onClick={()=>createSalesOrder({variables:{input:{quoteId:quote.id,paymentMode,advanceAmount:Number(advanceAmount||0),notes:'Created from quote detail'}}})}>Create sales order</Button>{quote.approvalStatus === 'pending' && <p className="mt-2 text-xs font-bold text-red-700">Owner approval required before conversion.</p>}</div>
      </aside>
    </section>
  </div>;
}
