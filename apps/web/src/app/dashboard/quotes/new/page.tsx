...
export default function QuoteBuilderPage() {
  ...
  return (
    <div className="grid h-[calc(100vh-10rem)] gap-5 overflow-hidden pb-4 xl:grid-cols-[1fr_0.52fr]">
      ...
      <section className="flex min-w-0 flex-col overflow-hidden rounded-[2.25rem] bg-[#ffffff]/72 shadow-2xl shadow-[#475569]/10 backdrop-blur-xl">
        <div className="border-b border-[#cbd5e1]/10 p-5 lg:p-6">
          <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.26em] text-[#475569]">Quote studio</p>
              <h1 className="mt-2 text-4xl font-black tracking-[-0.045em] text-[#0e1a3d]">Build a beautiful retail proposal.</h1>
              <p className="mt-2 text-sm font-semibold text-[#475569]">Search catalogue SKUs, add product-image rows, and save a quote version.</p>
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 custom-scrollbar lg:p-6">
          <div className="grid gap-4 lg:grid-cols-2">
            <label className="block space-y-2">
              <span className="text-xs font-black uppercase tracking-widest text-[#475569]">Customer</span>
              <select value={selectedCustomerId} onChange={(event) => setSelectedCustomerId(event.target.value)} className="h-[3.25rem] w-full rounded-2xl border border-[#cbd5e1]/18 bg-white px-4 text-sm font-bold text-[#0e1a3d] outline-none focus:border-[#2563eb]/45 focus:ring-4 focus:ring-[#2563eb]/10">
                <option value="">Select customer...</option>
                {customerData?.customers?.map((customer: any) => <option key={customer.id} value={customer.id}>{customer.name}</option>)}
              </select>
            </label>
            <label className="block space-y-2">
              <span className="text-xs font-black uppercase tracking-widest text-[#475569]">Project / site</span>
            ...
            <label className="block space-y-2">
              <span className="text-xs font-black uppercase tracking-widest text-[#475569]">PDF type</span>
              <select value={displayMode} onChange={(event) => setDisplayMode(event.target.value as any)} className="h-[3.25rem] w-full rounded-2xl border border-[#cbd5e1]/18 bg-white px-4 text-sm font-bold text-[#0e1a3d] outline-none focus:border-[#2563eb]/45 focus:ring-4 focus:ring-[#2563eb]/10">
                <option value="priced">Show prices - quotation</option>
                <option value="selection">Hide prices - selection summary</option>
              </select>
            </label>
            <label className="block space-y-2 lg:col-span-2">
              <span className="text-xs font-black uppercase tracking-widest text-[#475569]">Default area for new lines</span>
              <input
                list="mp-area-list"
                value={defaultArea}
                onChange={(event) => setDefaultArea(event.target.value)}
                placeholder="e.g. Master Bedroom Bathroom"
                className="h-[3.25rem] w-full rounded-2xl border border-[#cbd5e1]/18 bg-white px-4 text-sm font-bold text-[#0e1a3d] outline-none focus:border-[#2563eb]/45 focus:ring-4 focus:ring-[#2563eb]/10"
              />
              <div className="flex flex-wrap gap-2 pt-1">
                {AREA_SUGGESTIONS.slice(0, 8).map((area) => (
                  <button
                    ...
                    className={`rounded-full px-3 py-1 text-[10px] font-black uppercase tracking-wider transition ${defaultArea === area ? 'bg-[#0e1a3d] text-white' : 'bg-white/75 text-[#1e293b] hover:bg-[#dbeafe]'}`}
                  >{area}</button>
                ))}
              </div>
            </label>
          ...
          <div className="relative mt-6">
            <Search className="absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-[#475569]" />
            ...
            <AnimatePresence>
              {searchQuery.length >= 2 && searchData?.globalSearch?.products?.length > 0 && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute left-0 right-0 top-full z-40 mt-3 max-h-96 overflow-y-auto rounded-[1.75rem] border border-[#cbd5e1]/12 bg-[#ffffff] p-2 shadow-2xl custom-scrollbar">
                  {searchData.globalSearch.products.map((product: any) => (
                    <button key={product.id} onClick={() => addProduct(product)} className="flex w-full items-center justify-between gap-4 rounded-2xl p-3 text-left transition hover:bg-[#dbeafe]/65">
                      <div className="flex min-w-0 items-center gap-3">
                        ...
                        <div className="min-w-0"><p className="truncate text-sm font-black">{product.name}</p><p className="text-[10px] font-black uppercase tracking-wider text-[#475569]">{product.sku} · {product.brand}</p></div>
                      </div>
          ...
          <div className="mt-6 overflow-hidden rounded-[1.75rem] border border-[#cbd5e1]/12 bg-white/70">
            <table className="w-full min-w-[760px] text-left">
              <thead className="bg-[#dbeafe]/70 text-[10px] font-black uppercase tracking-widest text-[#cbd5e1]"><tr><th className="px-4 py-4">Product</th><th className="px-4 py-4 text-center">Qty</th><th className="px-4 py-4 text-right">Rate</th><th className="px-4 py-4 text-right">Amount</th><th className="px-4 py-4" /></tr></thead>
              <tbody className="divide-y divide-[#cbd5e1]/10">
                {lines.map((line) => (
                  <tr key={line.id}>
                    <td className="px-4 py-4"><div className="flex items-center gap-4"><ProductImageFrame src={line.quoteImage || productImage(line.media)} alt={line.name} className="h-24 w-28 shrink-0 rounded-[1.35rem]" imageClassName="p-1.5" /><div className="min-w-0 space-y-2"><input list="mp-area-list" value={line.area || ''} onChange={(event)=>updateLine(line.id,{area:event.target.value})} placeholder="Area / room" className="h-8 w-full rounded-xl border border-[#cbd5e1]/15 bg-white px-3 text-[10px] font-black uppercase tracking-wider text-[#2563eb]" /><p className="font-black">{line.name}</p><p className="text-[10px] font-black uppercase tracking-wider text-[#475569]">{line.sku} · {line.unit}</p><input value={line.quoteImage || ''} onChange={(event)=>updateLine(line.id,{quoteImage:event.target.value})} placeholder="Optional quote photo URL" className="h-8 w-full rounded-xl border border-[#cbd5e1]/15 bg-white px-3 text-[10px] font-bold" /></div></div></td>
                    <td className="px-4 py-4 text-center"><input type="number" value={line.qty} min={0} onChange={(event) => updateQty(line.id, Number(event.target.value) || 0)} className="h-10 w-20 rounded-xl border border-[#cbd5e1]/18 bg-white text-center text-sm font-black outline-none focus:ring-4 focus:ring-[#2563eb]/10" /></td>
                    ...
                    <td className="px-4 py-4"><button onClick={() => removeLine(line.id)} className="rounded-xl p-2 text-[#475569] hover:bg-red-50 hover:text-red-700"><Trash2 className="h-4 w-4" /></button></td>
                  </tr>
                ))}
              </tbody>
            </table>
            {lines.length === 0 && <div className="grid h-56 place-items-center text-center"><div><Plus className="mx-auto mb-3 h-8 w-8 text-[#2563eb]" /><p className="font-black text-[#0e1a3d]">Search products to start a quote.</p><p className="mt-1 text-sm font-semibold text-[#475569]">Catalogue images and price details will appear here.</p></div></div>}
          </div>
        </div>
      </section>

      <aside className="hidden overflow-hidden rounded-[2.25rem] bg-[#0e1a3d] p-5 text-white shadow-2xl shadow-[#0e1a3d]/15 xl:flex xl:flex-col">
        <div className="rounded-[1.75rem] bg-[#ffffff] p-5 text-[#0e1a3d]">
          <p className="text-[10px] font-black uppercase tracking-[0.24em] text-[#475569]">Live preview</p>
          <h2 className="mt-2 text-3xl font-black tracking-[-0.04em]">{projectTitle || 'Retail quotation'}</h2>
          <p className="mt-2 text-sm font-bold text-[#475569]">{selectedCustomer?.name || 'Select a customer'}</p>
        </div>
        <div className="mt-5 flex-1 overflow-y-auto rounded-[1.75rem] border border-white/10 bg-white/[0.08] p-4 custom-scrollbar">
          {lines.slice(0, 8).map((line) => <div key={line.id} className="mb-4 rounded-[1.5rem] bg-white/10 p-3"><ProductImageFrame src={productImage(line.media)} alt={line.name} className="mb-3 h-36 w-full rounded-[1.25rem]" imageClassName="p-2" /><div className="min-w-0"><p className="line-clamp-2 text-sm font-black">{line.name}</p><p className="mt-1 text-[10px] font-black uppercase text-[#bfdbfe]">{line.sku} · {line.qty} x {money(line.price)}</p></div></div>)}
          {lines.length === 0 && <div className="grid h-full place-items-center text-center text-[#dbeafe]"><div><ImageIcon className="mx-auto mb-3 h-10 w-10" /><p className="text-sm font-bold">Product image preview appears after adding items.</p></div></div>}
        </div>
        <div className="mt-5 rounded-[1.75rem] bg-[#ffffff] p-5 text-[#0e1a3d]">
          ...
          <div className="mt-4 flex justify-between border-t border-[#cbd5e1]/12 pt-4 text-2xl font-black"><span>Total</span><span>{money(total)}</span></div>
        </div>
      </aside>
    </div>
  );
