...
export default function MasterDataIndex(){return <div className="space-y-7 pb-10"><section className="rounded-[2.25rem] bg-[#241b14] p-7 text-white"><p className="text-[10px] font-black uppercase tracking-[0.26em] text-[#ead7bd]">Master data modules</p><h1 className="mt-3 text-5xl font-black tracking-[-0.05em]">Separate control rooms for each master function.</h1><p className="mt-3 max-w-2xl text-sm font-semibold leading-6 text-[#f6eadb]">Catalogue browsing, SKU creation, imports, vendors, customers and settings are split so role permissions stay clean.</p></section><section className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{modules.map(([title,body,href,Icon]: any)=><Link href={href} key={href} className="mp-card rounded-[2rem] p-6 transition hover:-translate-y-1 hover:shadow-2xl"><Icon className="h-8 w-8 text-[#b17643]"/><h2 className="mt-5 text-2xl font-black text-[#241b14]">{title}</h2><p className="mt-2 text-sm font-bold leading-6 text-[#6f6258]">{body}</p></Link>)}</section></div>}
