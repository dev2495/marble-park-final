...
import { Archive, ArrowLeft, ArrowRight, Boxes, ChevronLeft, ChevronRight, History, ImagePlus, PackagePlus, Plus, Save, ScanLine, Trash2 } from 'lucide-react';
...
const MRP_HISTORY = gql`query ProductMrpHistory($productId:String){ productMrpHistoryPage(productId:$productId,take:12) }`;
...
export default function ProductMasterPage() {
  ...
  const { data: historyData, refetch: refetchHistory } = useQuery(MRP_HISTORY, { variables: { productId: selectedId || undefined }, skip: !selectedId, fetchPolicy: 'network-only' });
  ...
  async function saveProduct() {
    ...
    try {
      ...
      await refetch();
      if (selectedId) await refetchHistory();
    } catch (error: any) {
    ...
    <section className="grid min-w-0 gap-6 xl:grid-cols-[1.1fr_0.9fr]">
      <div className="mp-card min-w-0 rounded-r5 p-5">
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-2">
          ...
          <label className="space-y-2"><span className="text-xs font-medium uppercase tracking-widest text-[#52525b]">Category *</span><SearchableSelect value={form.category} onValueChange={changeCategory} options={categories.map((item: any) => ({ value: item.name, label: item.name, description: item.code ? `Code ${item.code}` : undefined }))} placeholder="Select category" searchPlaceholder="Search categories…" /></label>
          ...
          <div className="grid gap-3 rounded-xl bg-[#f8f4ef] p-4 md:col-span-2 md:grid-cols-2 xl:grid-cols-3">
            ...
            {mrpChanged ? <label className="space-y-2 md:col-span-2 xl:col-span-3"><span className="text-xs font-medium uppercase tracking-widest text-[#9f342d]">Why is this MRP changing? *</span><Input value={mrpChangeReason} onChange={(event) => setMrpChangeReason(event.target.value)} placeholder="Example: Brand price list revised from 1 September" /><span className="block text-[11px] leading-5 text-[#71717a]">This reason, old MRP, new MRP, user, source, UOM and effective date are kept permanently in the SKU MRP history.</span></label> : null}
          </div>
          {isEditing ? <section className="rounded-xl border border-[#ead8cf] bg-white p-4 md:col-span-2"><div className="flex items-center justify-between gap-3"><div><p className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.16em] text-[#9f342d]"><History className="h-4 w-4" />MRP history</p><p className="mt-1 text-xs text-[#71717a]">Append-only changes for this permanent SKU.</p></div><Link href={`/dashboard/audit?entityType=Product&entityId=${selectedId}&tab=mrp`} className="text-xs font-black text-[#9f342d] underline underline-offset-4">Open full audit</Link></div><div className="mt-3 divide-y divide-[#f0e4de]">{mrpHistory.map((entry:any)=><div key={entry.id} className="grid gap-1 py-2 text-xs sm:grid-cols-[1fr_auto] sm:items-center"><div><p className="font-bold text-[#18181b]">{entry.previousMrpInclusive==null?'Initial MRP':`₹${Number(entry.previousMrpInclusive).toLocaleString('en-IN')} →`} ₹{Number(entry.newMrpInclusive).toLocaleString('en-IN')} / {entry.priceUom}</p><p className="mt-1 text-[#71717a]">{entry.reason} · {entry.actor?.name||entry.changedById}</p></div><time className="font-semibold text-[#71717a]">{new Date(entry.createdAt).toLocaleString('en-IN')}</time></div>)}{!mrpHistory.length?<p className="py-3 text-xs font-semibold text-[#71717a]">No MRP history is available yet. The next MRP revision will start the governed trail.</p>:null}</div></section> : null}
