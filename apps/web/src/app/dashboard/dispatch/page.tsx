        ...
        <div className="relative flex flex-col justify-between gap-6 lg:flex-row lg:items-end">
          <div>
            ...
            <p className="mt-3 max-w-3xl text-sm font-medium leading-6 text-[var(--ink-3)]">
              The board separates ready rows from confirmed-but-not-inward rows. Pending-inward items stay blocked until Procurement PO and GRN receiving make them dispatchable.
            </p>
          </div>
          <div className="flex flex-wrap gap-3">
            ...
            <Button asChild size="lg">
              <Link href="/dashboard/procurement"><Package className="mr-2 h-5 w-5" /> Procurement / GRN</Link>
            </Button>
          </div>
        </div>
                  ...
                  <motion.article key={job.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.03 }} className="rounded-r4 border border-[var(--line)] bg-white/85 p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-sm-soft">
                    ...
                    <div className="mt-4 space-y-2">
                      {jobLines(job).filter((line: any) => Number(line.remainingQty || 0) > 0).map((line: any, lineIndex: number) => {
                        ...
                        const tileSpecial = line.status === 'tile_special_order';
                        return (
                          <div key={key} className="rounded-r4 border border-[var(--line)] bg-[var(--surface)] p-3">
                            <div className="flex items-start justify-between gap-3">
                              ...
                              <span className={ready ? 'rounded-full bg-emerald-50 px-2 py-1 text-[10px] font-black uppercase text-emerald-700' : invalid ? 'rounded-full bg-red-50 px-2 py-1 text-[10px] font-black uppercase text-red-700' : tileSpecial ? 'rounded-full bg-blue-50 px-2 py-1 text-[10px] font-black uppercase text-blue-700' : 'rounded-full bg-amber-50 px-2 py-1 text-[10px] font-black uppercase text-amber-700'}>
                                {ready ? 'Ready' : invalid ? 'Rebuild quote' : tileSpecial ? 'Tile order' : 'Pending inward'}
                              </span>
                            </div>
                            {ready ? (
                              ...
                              <p className="mt-3 text-xs font-semibold text-blue-700">Tile lines are tracked by code/size and stay blocked here until their PO/GRN receipt is posted.</p>
                            ) : (
