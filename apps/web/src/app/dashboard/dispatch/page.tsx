    ...
    <div className="space-y-7 pb-10">
      ...
      <section className="rounded-[2.25rem] bg-[#241b14] p-7 text-white shadow-2xl shadow-[#241b14]/15">
        <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.26em] text-[#ead7bd]">Dispatch control</p>
            <h1 className="mt-3 text-5xl font-black tracking-[-0.05em]">Pack, challan, dispatch, close.</h1>
            <p className="mt-3 max-w-2xl text-sm font-semibold leading-6 text-[#f6eadb]">A reservation-aware board for daily loads, pending challans and site delivery state.</p>
          </div>
          <Button size="lg" className="bg-[#ffffff] text-[#241b14] hover:bg-white"><Truck className="mr-2 h-5 w-5" /> Manual dispatch</Button>
        </div>
      </section>
      ...
      <section className="grid grid-cols-1 gap-5 xl:grid-cols-4">
        {columns.map((column, index) => {
          ...
          return (
            <div key={column.id} className="min-h-[560px] rounded-[2rem] border border-[#d9cbbd]/12 bg-[#ffffff]/70 p-4 shadow-xl shadow-[#6f6258]/8 backdrop-blur">
              <div className="flex items-center justify-between px-1 pb-4">
                <div className="flex items-center gap-3">
                  <div className="grid h-10 w-10 place-items-center rounded-2xl bg-[#f6eadb] text-[#8a552e]"><column.icon className="h-5 w-5" /></div>
                  <h2 className="font-black text-[#241b14]">{column.title}</h2>
                </div>
                <span className="rounded-full bg-white/70 px-2.5 py-1 text-xs font-black text-[#8a552e]">{rows.length}</span>
              </div>
              <div className="space-y-3">
                {loading ? <p className="rounded-2xl bg-white/60 p-4 text-sm font-bold text-[#6f6258]">Loading...</p> : rows.map((job: any) => (
                  <motion.article key={job.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.03 }} className="rounded-[1.5rem] bg-white/80 p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-xl">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="text-[10px] font-black uppercase tracking-widest text-[#6f6258]">{job.id}</p>
                        <h3 className="mt-1 text-base font-black text-[#241b14]">{job.customer?.name || 'Dispatch job'}</h3>
                      </div>
                      <FileText className="h-5 w-5 text-[#b17643]" />
                    </div>
                    <p className="mt-3 flex items-start gap-2 text-xs font-semibold leading-5 text-[#6f6258]"><MapPin className="mt-0.5 h-4 w-4 shrink-0" />{job.siteAddress || 'Site address pending'}</p>
                    <div className="mt-4 rounded-2xl bg-[#f6eadb]/70 px-3 py-2 text-[10px] font-black uppercase tracking-wider text-[#8a552e]">Quote {job.quote?.quoteNumber || job.quoteId || 'manual'}</div>
                    <div className="mt-4 flex flex-wrap gap-2">
                      {((Array.isArray(job.quote?.lines) ? job.quote.lines : []) as any[]).map((line: any) => {
                        ...
                        return <div key={key} className="w-full rounded-2xl bg-[#ffffff] p-3">
                          <p className="text-xs font-black text-[#241b14]">{line.name || line.sku}</p>
                          <p className="mt-1 text-[10px] font-black uppercase tracking-wider text-[#6f6258]">Remaining {remaining} of {totalQty}</p>
                          <div className="mt-2 flex gap-2">
                            <input type="number" min={1} max={remaining} value={dispatchQty[key] || remaining} onChange={(event) => setDispatchQty({ ...dispatchQty, [key]: Number(event.target.value) || 1 })} className="h-9 w-20 rounded-xl border border-[#d9cbbd]/15 bg-white px-2 text-center text-sm font-black" />
                      ...
                      {challans.filter((challan: any) => challan.dispatchJobId === job.id).map((challan: any) => (
                        <div key={challan.id} className="flex flex-wrap gap-2">
                          <span className="rounded-2xl bg-[#241b14] px-3 py-2 text-[10px] font-black uppercase tracking-wider text-white">{challan.challanNumber} · {challan.status}</span>
                ...
                {!loading && rows.length === 0 && <div className="grid h-40 place-items-center rounded-[1.5rem] border border-dashed border-[#d9cbbd]/20 text-center text-sm font-bold text-[#6f6258]">No jobs</div>}
              </div>
            </div>
          );
        })}
