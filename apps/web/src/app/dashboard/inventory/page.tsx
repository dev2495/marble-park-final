...
export default function InventoryPage() {
  ...
  const stats = [
    { label: 'Stock valuation', value: money(dashboard.totalValue || 0), icon: ShieldCheck, tone: 'text-[#059669]' },
    { label: 'Available qty', value: (dashboard.totalAvailable || 0).toLocaleString('en-IN'), icon: Boxes, tone: 'text-[#059669]' },
  ...
  return (
    <div className="space-y-7 pb-10">
      <section className="grid gap-5 lg:grid-cols-[0.82fr_1.18fr]">
        <div className="mp-card relative overflow-hidden rounded-r6 p-7 text-[#18181b]">
        ...
        <div className="grid grid-cols-2 gap-4">
          {stats.map((stat, index) => (
            <motion.div key={stat.label} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} className="mp-card rounded-r5 p-5">
              <div className={`grid h-12 w-12 place-items-center rounded-2xl bg-[#eff6ff] ${stat.tone}`}><stat.icon className="h-6 w-6" strokeWidth={1.5} /></div>
              <div className="mt-5 text-3xl font-black tracking-tight text-[#18181b]">{stat.value}</div>
              <div className="mt-1 text-sm font-black text-[#52525b]">{stat.label}</div>
            </motion.div>
          ))}
        </div>
      </section>
        ...
        <section className="mp-card rounded-r5 border border-red-200/70 bg-red-50/60 p-5">
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-semibold text-[#18181b]">{lowStockRows.length} SKU{lowStockRows.length === 1 ? '' : 's'} need re-ordering</h2>
              <p className="mt-1 text-sm font-bold text-[#52525b]">Available stock has dropped to or below each product's threshold (or explicit reorder point if set).</p>
            </div>
          </div>
                ...
                {lowStockRows.map((row: any) => (
                  <tr key={row.id} className="text-sm">
                    <td className="px-3 py-2">
                      <div className="font-semibold text-[#18181b]">{row.product?.name}</div>
                      <div className="text-xs font-medium uppercase tracking-wider text-[#52525b]">{row.product?.sku} · {row.product?.brand}</div>
                    </td>
                    ...
                    <td className="px-3 py-2 text-right text-xs font-bold text-[#52525b]">{row.updatedAt ? new Date(row.updatedAt).toLocaleDateString() : '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <section className="mp-card rounded-r5 p-4 lg:p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <p className="text-sm font-bold text-[#52525b]">Showing {balances.length.toLocaleString('en-IN')} rows</p>
        </div>

        <div className="mt-5 overflow-hidden rounded-r4 border border-[#e4e4e7]/12 bg-white/72">
          <div className="overflow-x-auto custom-scrollbar">
            <table className="w-full min-w-[860px] text-left">
              <thead className="bg-[#eff6ff]/65 text-xs font-medium uppercase tracking-widest text-[#cbd5e1]">
                <tr>
              ...
              <tbody className="divide-y divide-[#cbd5e1]/10">
                {loading ? (
                  <tr><td colSpan={6} className="px-5 py-12 text-center text-sm font-bold text-[#52525b]">Loading inventory...</td></tr>
                  ...
                  <tr key={item.id} className="transition hover:bg-white">
                    ...
                    <td className="px-5 py-4 text-sm font-bold text-[#27272a]">{item.product?.category}</td>
                    <td className="px-5 py-4 text-center font-black">{item.onHand}</td>
                    <td className="px-5 py-4 text-center"><span className="rounded-full bg-[#ecfdf5] px-3 py-1 text-sm font-black text-[#059669]">{item.available}</span></td>
