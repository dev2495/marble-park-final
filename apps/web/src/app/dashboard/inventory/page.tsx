...
const GET_LOW_STOCK = gql`
  query LowStockBalances($take: Int) {
    lowStockBalances(take: $take) {
      ...
      reorderPoint
      isLowStock
      product { id sku name brand category defaultNrpInclusive media }
      updatedAt
    }
  }
`;
...
export default function InventoryPage() {
  ...
  const canViewCost = ['admin', 'owner'].includes(meData?.me?.role || '');
  const canCorrectCost = canViewCost;
  ...
  const stats = [
    ...
    [AlertTriangle, 'Data exceptions', qty(Number(summary.zeroDefaultNrp || 0) + Number(summary.zeroCostOnHand || 0)), 'missing default NRP or lot cost', 'text-rose-700', 'data_exception'],
  ] as const;
