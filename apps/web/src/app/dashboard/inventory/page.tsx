...
export default function InventoryPage() {
  ...
  const stats = [
    ...
    { label: 'Reserved qty', value: (dashboard.totalReserved || 0).toLocaleString('en-IN'), icon: Warehouse, tone: 'text-[#2563eb]' },
    ...
    <div className="space-y-7 pb-10">
      <section className="grid gap-5 lg:grid-cols-[0.82fr_1.18fr]">
        <div className="mp-dark-panel relative overflow-hidden rounded-[2.25rem] p-7 text-white">
          ...
          <div className="relative">
            <p className="text-[10px] font-black uppercase tracking-[0.26em] text-[#bfdbfe]">Inventory truth</p>
            <h1 className="mt-4 text-5xl font-black leading-[0.95] tracking-[-0.05em]">Stock desk for sales, inward and dispatch.</h1>
            <p className="mt-5 text-sm font-semibold leading-6 text-[#dbeafe]">Catalogue-only SKUs are visible with zero stock until inwarded. This avoids fake stock while keeping every quoted item searchable.</p>
            <Button asChild size="lg" className="mt-7 bg-[#ffffff] text-[#0e1a3d] hover:bg-white"><Link href="/dashboard/inventory/inwards"><PackagePlus className="mr-2 h-5 w-5" /> New inward</Link></Button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          {stats.map((stat, index) => (
            <motion.div key={stat.label} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} className="mp-card rounded-[2rem] p-5">
              <div className={`grid h-12 w-12 place-items-center rounded-2xl bg-[#dbeafe] ${stat.tone}`}><stat.icon className="h-6 w-6" strokeWidth={1.5} /></div>
              <div className="mt-5 text-3xl font-black tracking-tight text-[#0e1a3d]">{stat.value}</div>
              <div className="mt-1 text-sm font-black text-[#475569]">{stat.label}</div>
            </motion.div>
          ))}
        </div>
      </section>
        ...
        <section className="mp-card rounded-[2rem] border border-red-200/70 bg-red-50/60 p-5">
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-black text-[#0e1a3d]">{lowStockRows.length} SKU{lowStockRows.length === 1 ? '' : 's'} need re-ordering</h2>
              <p className="mt-1 text-sm font-bold text-[#475569]">Available stock has dropped to or below each product's threshold (or explicit reorder point if set).</p>
            </div>
          </div>
                ...
                {lowStockRows.map((row: any) => (
                  <tr key={row.id} className="text-sm">
                    <td className="px-3 py-2">
                      <div className="font-black text-[#0e1a3d]">{row.product?.name}</div>
                      <div className="text-[10px] font-black uppercase tracking-wider text-[#475569]">{row.product?.sku} · {row.product?.brand}</div>
                    </td>
                    ...
                    <td className="px-3 py-2 text-right text-xs font-bold text-[#475569]">{row.updatedAt ? new Date(row.updatedAt).toLocaleDateString() : '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <section className="mp-card rounded-[2rem] p-4 lg:p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <p className="text-sm font-bold text-[#475569]">Showing {balances.length.toLocaleString('en-IN')} rows</p>
        </div>

        <div className="mt-5 overflow-hidden rounded-[1.5rem] border border-[#cbd5e1]/12 bg-white/72">
          <div className="overflow-x-auto custom-scrollbar">
            <table className="w-full min-w-[860px] text-left">
              <thead className="bg-[#dbeafe]/65 text-[10px] font-black uppercase tracking-widest text-[#cbd5e1]">
                <tr>
              ...
              <tbody className="divide-y divide-[#cbd5e1]/10">
                {loading ? (
                  <tr><td colSpan={6} className="px-5 py-12 text-center text-sm font-bold text-[#475569]">Loading inventory...</td></tr>
                ) : balances.map((item: any) => (
                  <tr key={item.id} className="transition hover:bg-[#ffffff]">
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        <img src={item.product?.media?.primary || '/catalogue-art/faucet.svg'} alt="" className="h-12 w-12 rounded-2xl bg-[#dbeafe] object-contain p-1" />
                        <div className="min-w-0">
                          <div className="truncate text-sm font-black text-[#0e1a3d]">{item.product?.name}</div>
                          <div className="text-[10px] font-black uppercase tracking-wider text-[#475569]">{item.product?.sku} · {item.product?.brand}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-5 py-4 text-sm font-bold text-[#1e293b]">{item.product?.category}</td>
                    ...
                    <td className="px-5 py-4 text-center font-black text-[#2563eb]">{item.reserved}</td>
