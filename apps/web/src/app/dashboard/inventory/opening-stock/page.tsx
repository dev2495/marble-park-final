...
const POST = gql`mutation PostOpening($id: ID!) { approveOpeningStockSession(id: $id) }`;
...
export default function OpeningStockPage() {
  ...
  async function submit() {
    setMessage('');
    try {
      await create({ variables: { input: { locationId: selectedLocation, effectiveAt: new Date(`${effectiveAt}T12:00:00`).toISOString(), submit: true, lines: JSON.stringify(lines.map(({ sku, internalCode, name, ...row }) => ({ ...row, quantity: Number(row.quantity), unitCost: Number(row.unitCost || 0), packCount: Number(row.packCount || 0) }))) } } });
  ...
  return <div className="space-y-5 pb-10">
    ...
    <section className="mp-panel overflow-hidden"><div className="border-b border-[var(--line)] p-4"><h2 className="font-semibold text-[var(--ink)]">Opening sessions</h2></div><div className="divide-y divide-[var(--line)]">{sessions.map((session: any) => <div key={session.id} className="flex flex-wrap items-center justify-between gap-3 p-4"><div><p className="font-semibold text-[var(--ink)]">{session.sessionNumber} · {session.location?.name}</p><p className="text-xs text-[var(--ink-4)]">{session.lines?.length || 0} lots · {session.status}</p></div>{session.status !== 'posted' ? <Button size="sm" disabled={posting} onClick={() => post({ variables: { id: session.id } })}><CheckCircle2 className="mr-2 h-4 w-4"/>Post stock & labels</Button> : <span className="text-xs font-semibold text-emerald-700">Posted</span>}</div>)}</div></section>
  </div>;
}
