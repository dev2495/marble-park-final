...
import { Barcode, Box, Check, ChevronLeft, ChevronRight, ClipboardCheck, HelpCircle as CircleHelp, History, PackageCheck, PackageSearch, Printer, QrCode, ScanLine, Search, Sparkles, Store, XCircle } from 'lucide-react';
...
export default function LabelDeskPage() {
  ...
  return <div className="space-y-5 pb-12">
    ...
    {tab==='scan'?<section className="grid gap-5 lg:grid-cols-[1fr_22rem]"><div className="overflow-hidden rounded-2xl border border-[var(--line)] bg-[var(--surface)]"><div className="bg-[linear-gradient(120deg,#1d2928,#214e48)] p-6 text-white"><div className="flex items-center gap-2"><ScanLine className="h-5 w-5 text-emerald-300"/><h2 className="text-lg font-bold">Scan and verify physical identity</h2></div><p className="mt-2 max-w-2xl text-sm leading-6 text-white/70">Scan the actual branded QR or type its human code. The result shows the linked SKU, exact lot or display asset before any downstream action.</p><div className="mt-5 flex gap-2"><Input autoFocus autoCapitalize="characters" className="border-white/20 bg-white text-slate-950" value={scanCode} onChange={(event)=>setScanCode(event.target.value)} onKeyDown={(event)=>{if(event.key==='Enter'&&scanCode.trim())scan({variables:{labelCode:scanCode,input:{action:'stock_lookup'}}});}} placeholder="Scan MP-LABEL payload or label code…"/><Button className="bg-emerald-500 text-emerald-950 hover:bg-emerald-400" aria-label="Scan label" disabled={scanState.loading||!scanCode.trim()} onClick={()=>scan({variables:{labelCode:scanCode,input:{action:'stock_lookup'}}})}><ScanLine className="h-4 w-4"/></Button></div></div>{scanResult?<div className={`m-5 rounded-2xl border p-5 ${scanResult.result==='success'?'border-emerald-200 bg-emerald-50':'border-red-200 bg-red-50'}`}><div className="flex items-center gap-2"><span className={`grid h-8 w-8 place-items-center rounded-full text-white ${scanResult.result==='success'?'bg-emerald-600':'bg-red-600'}`}>{scanResult.result==='success'?<Check className="h-4 w-4"/>:<XCircle className="h-4 w-4"/>}</span><p className="font-bold capitalize">{scanResult.result.replaceAll('_',' ')}</p></div>{scanResult.label?<div className="mt-4 flex gap-4"><ProductImageFrame src={imageOf(scanResult.label)} alt={scanResult.label.product?.name||'Scanned item'} className="h-20 w-20 shrink-0 rounded-xl"/><div><p className="font-bold">{scanResult.label.displaySample?.internalCode||scanResult.label.product?.internalCode||scanResult.label.product?.sku} · {scanResult.label.product?.name}</p><p className="mt-1 text-sm text-[var(--ink-3)]">{scanResult.label.lot?.lotNumber||scanResult.label.displaySample?.sampleNumber||'Product / shelf identity'}</p><p className="mt-2 font-mono text-xs text-[var(--ink-4)]">{scanResult.label.labelCode}</p></div></div>:<p className="mt-3 text-sm">No registered label matches this payload. Do not move or dispatch the item.</p>}</div>:<Empty icon={QrCode} title="Ready for the next scan" text="The branded MP mark is visual; the encoded payload remains a standards-valid MP-LABEL identifier."/>}</div><aside className="space-y-4"><div className="rounded-2xl border border-amber-200 bg-amber-50 p-5"><ClipboardCheck className="h-5 w-5 text-amber-700"/><h3 className="mt-3 font-bold text-amber-950">Before moving stock</h3><p className="mt-2 text-xs leading-5 text-amber-900">Match the human code, product, lot, batch and location. A display label identifies a non-sellable asset and must never be used as stock.</p></div><Button asChild variant="outline" className="w-full"><Link href="/dashboard/help#labels-lots"><CircleHelp className="mr-2 h-4 w-4"/>Open scan help</Link></Button></aside></section>:null}
  </div>;
}
