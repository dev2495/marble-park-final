...
import {
  Bath, Bell, BookOpen, Boxes, Briefcase, ChevronDown, ClipboardCheck, ClipboardList, FileSpreadsheet,
  History, KeyRound, LayoutDashboard, ListChecks, LogOut, Menu, PackageSearch, Receipt, RefreshCw,
  Search, Settings, Shield, Target, Truck, UserCircle2, Users, UserCog, Wallet, X,
} from 'lucide-react';
...
import { usePageHelpTopic } from '@/components/help/use-page-help';
...
const navSections: Array<{ title: string; items: Array<{ name: string; href: string; icon: any; roles: string[]; permission?: string }> }> = [
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'Payments', href: '/dashboard/payments', icon: Wallet, roles: ['admin', 'owner', 'sales_manager', 'office_staff'], permission: 'payments.manage' },
      { name: 'Returns', href: '/dashboard/returns', icon: RefreshCw, roles: ['admin', 'owner', 'sales_manager', 'office_staff', 'inventory_manager', 'dispatch_ops'], permission: 'returns.manage' },
    ],
  },
  {
    title: 'Stock',
    items: [
      ...
      { name: 'Pending Inward', href: '/dashboard/pending-inward', icon: PackageSearch, roles: ['admin', 'owner', 'sales_manager', 'sales', 'inventory_manager', 'dispatch_ops', 'office_staff'], permission: 'goods_receipts.manage' },
      ...
      { name: 'Procurement', href: '/dashboard/procurement', icon: ClipboardList, roles: ['admin', 'owner', 'inventory_manager', 'sales_manager', 'office_staff'], permission: 'procurement.manage' },
    ],
  },
  {
    title: 'People & Data',
    items: [
      ...
      { name: 'Sales Targets', href: '/dashboard/targets', icon: Target, roles: ['admin', 'owner', 'sales_manager', 'sales'] },
      { name: 'Reports', href: '/dashboard/reports', icon: FileSpreadsheet, roles: ['admin', 'owner', 'sales_manager', 'inventory_manager', 'office_staff'], permission: 'reports.view' },
      { name: 'Audit Log', href: '/dashboard/audit', icon: History, roles: ['admin', 'owner'], permission: 'audit.view' },
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'Help Center', href: '/help', icon: BookOpen, roles: ['admin', 'owner', 'sales_manager', 'sales', 'inventory_manager', 'dispatch_ops', 'office_staff'] },
    ],
  },
];
...
const pageTitles: Record<string, string> = {
  ...
  '/dashboard/payments': 'Payments & Refunds',
  '/dashboard/returns': 'Returns Desk',
  '/dashboard/reports': 'Reports',
  '/dashboard/targets': 'Sales Targets',
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const helpTopicId = usePageHelpTopic();
  ...
  const previewingRole = user?.role === 'admin' && Boolean(roleOverride);
  const effectivePermissions = previewingRole ? [] : (me?.effectivePermissions || user?.effectivePermissions || []);
  const canSeeItem = (item: { roles: string[]; permission?: string }) =>
    item.roles.includes(effectiveRole) || Boolean(item.permission && effectivePermissions.includes(item.permission));
