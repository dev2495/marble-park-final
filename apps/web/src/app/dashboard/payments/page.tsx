  ...
  return <div className="space-y-5 pb-10">
    <header className="border-b border-[var(--line)] pb-5">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2"><Button variant="outline" onClick={() => refetch()}><RefreshCw className="mr-2 h-4 w-4" />Refresh</Button><Button onClick={() => setPaymentCustomer(accounts[0]?.customer || null)} disabled={!accounts.length}><Plus className="mr-2 h-4 w-4" />Record payment</Button></div>
      </div>
    </header>
    ...
    {paymentCustomer ? <PaymentForm customer={paymentCustomer} account={activeCustomer?.id === paymentCustomer.id ? account : null} busy={paymentState.loading} onClose={() => setPaymentCustomer(null)} onSubmit={submitPayment} /> : null}
