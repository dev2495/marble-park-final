...
const DATA = gql`query Leads($search: String, $stage: String, $skip: Float, $take: Float) { leads(search: $search, stage: $stage, skip: $skip, take: $take) { id title source stage expectedValue notes nextActionAt customer owner } }`;
...
export default function LeadsPipelinePage() {
  ...
  const pageSize = 30;
  const { data, loading, error, refetch } = useQuery(DATA, { variables: { search: search || undefined, stage: stage === 'all' ? undefined : stage, skip: page * pageSize, take: pageSize + 1 }, fetchPolicy: 'cache-and-network' });
  const [updateStage, updateState] = useMutation(UPDATE_STAGE, { onCompleted: () => refetch() });
  const leadPage = useMemo<any[]>(() => data?.leads || [], [data?.leads]);
  const leads = leadPage.slice(0, pageSize);
  const hasNext = leadPage.length > pageSize;
  const rows = leads;
  ...
  return <div className="space-y-5 pb-10">
    ...
    <section className="grid gap-4 border-b border-[var(--line)] pb-4 sm:grid-cols-[1fr_auto_auto] sm:items-end"><label className="flex h-10 max-w-lg items-center rounded-md border border-[var(--line)] bg-[var(--bg-soft)] px-3"><Search className="mr-2 h-4 w-4 text-[var(--ink-4)]"/><input value={search} onChange={(event) => { setSearch(event.target.value); setPage(0); }} placeholder="Customer, project or requirement" className="w-full bg-transparent text-sm outline-none"/></label><div><p className="text-xl font-semibold tabular-nums text-[var(--ink)]">{money(pipelineValue)}</p><p className="text-xs text-[var(--ink-4)]">Visible open pipeline</p></div><div><p className={`text-xl font-semibold tabular-nums ${overdue ? 'text-red-700' : 'text-emerald-700'}`}>{overdue}</p><p className="text-xs text-[var(--ink-4)]">Visible overdue</p></div></section>
    <div className="flex gap-1 overflow-x-auto rounded-md border border-[var(--line)] bg-[var(--surface)] p-1">{stages.map((item) => <button key={item} onClick={() => { setStage(item); setPage(0); }} className={`h-9 min-w-fit rounded px-3 text-xs font-semibold capitalize ${stage === item ? 'bg-[var(--ink)] text-white' : 'text-[var(--ink-3)] hover:bg-[var(--bg-soft)]'}`}>{item === 'proposal' ? 'Quoted' : item}</button>)}</div>
