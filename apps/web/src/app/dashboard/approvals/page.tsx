...
export default function ApprovalsPage() {
  ...
  return <div className="space-y-6 pb-10">
    ...
    {!loading && totalPending === 0 && <div className="mp-card grid min-h-64 place-items-center rounded-[2rem] p-8 text-center"><div><ShieldAlert className="mx-auto h-10 w-10 text-[#b57942]" /><h2 className="mt-4 text-2xl font-black text-[#211b16]">No approvals pending</h2><p className="mt-2 text-sm font-bold text-[#8b6b4c]">Approval requests will appear here automatically.</p></div></div>}
    {tab === 'quotes' && <section className="space-y-3">{quotes.map((quote: any) => <article key={quote.id} className="mp-card grid gap-4 rounded-[2rem] p-5 lg:grid-cols-[1fr_0.7fr_0.55fr_auto] lg:items-center"><div><Link href={`/dashboard/quotes/${quote.id}`} className="text-xl font-black text-[#211b16] hover:underline">{quote.quoteNumber}</Link><p className="mt-1 text-sm font-bold text-[#7d6b5c]">{quote.title || quote.projectName || 'Retail quotation'}</p>{quote.approval?.availabilityIssues?.length > 0 && <p className="mt-2 rounded-2xl bg-amber-50 px-3 py-2 text-xs font-black text-amber-800">Availability warning: {quote.approval.availabilityIssues.length} short item(s)</p>}</div><div><p className="font-black text-[#211b16]">{quote.customer?.name || 'Customer'}</p><p className="text-xs font-bold text-[#8b6b4c]">Owner: {quote.owner?.name || 'Unassigned'}</p></div><div className="text-right"><p className="text-2xl font-black text-[#211b16]">{money(total(quote.lines))}</p><p className="text-xs font-black uppercase tracking-wider text-[#b57942]">Discount {quote.discountPercent || 0}%</p></div><Button disabled={approvingQuote} onClick={() => approveQuote({ variables: { id: quote.id, note: 'Approved from owner approval desk' } })}><CheckCircle2 className="mr-2 h-4 w-4" /> Approve quote</Button></article>)}</section>}
    {tab === 'imports' && <section className="space-y-3">{imports.map((batch: any) => <article key={batch.id} className="mp-card flex flex-wrap items-center justify-between gap-4 rounded-[2rem] p-5"><div><p className="text-xl font-black text-[#211b16]">{batch.brand || 'Catalogue import'} · {batch.rowCount} rows</p><p className="mt-1 text-xs font-bold text-[#8b6b4c]">{batch.id} · {batch.status}</p></div><Button disabled={approvingImport} onClick={() => approveImport({ variables: { importBatchId: batch.id, note: 'Approved from owner approval desk' } })}><CheckCircle2 className="mr-2 h-4 w-4" /> Approve import</Button></article>)}</section>}
    {tab === 'images' && <section className="space-y-3">{catalogTasks.map((task: any) => <article key={task.id} className="mp-card flex flex-wrap items-center justify-between gap-4 rounded-[2rem] p-5"><div className="flex items-center gap-4">{task.imageUrl && <img src={task.imageUrl} alt="catalogue task" className="h-24 w-28 rounded-2xl bg-white object-contain" />}<div><p className="text-xl font-black text-[#211b16]">{task.detectedSku || 'Image mapping'} · {task.status}</p><p className="mt-1 text-xs font-bold text-[#8b6b4c]">Product: {task.mappedProductId || 'not mapped'} · {task.id}</p></div></div><Button disabled={approvingCatalog || !task.mappedProductId} onClick={() => approveCatalog({ variables: { id: task.id, note: 'Approved from owner approval desk' } })}><CheckCircle2 className="mr-2 h-4 w-4" /> Approve image</Button></article>)}</section>}
  </div>;
}
