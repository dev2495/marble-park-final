...
export default function ApprovalsPage() {
  ...
  return <div className="space-y-6 pb-10">
    ...
    <section className="mp-card flex flex-wrap gap-2 rounded-r5 p-3">{[['quotes','Quote approvals',quotes.length,FileSpreadsheet],['imports','Import approvals',imports.length,Database],['images','Image approvals',catalogTasks.length,FileImage]].map(([id,label,count,Icon]: any)=><button key={id} onClick={()=>setTab(id)} className={`flex items-center gap-2 rounded-2xl px-4 py-3 text-sm font-black ${tab===id?'bg-[var(--surface-3)] text-[var(--bg)]':'bg-[var(--surface)] text-[var(--ink-2)] hover:bg-[var(--bg-soft)]'}`}><Icon className="h-4 w-4"/>{label}<span className="rounded-full bg-[var(--brand-50)] px-2 py-0.5 text-[10px] text-[var(--brand-700)]">{count}</span></button>)}</section>
