...
export default function LabelPrintPage({ params }: { params: Promise<{ runId: string }> }) {
  ...
  return <main className="min-h-screen bg-slate-100 p-4 text-black print:bg-white print:p-0">
    <style dangerouslySetInnerHTML={{__html:printCss}}/>
    {error?<div className="print-controls mx-auto max-w-3xl"><QueryErrorBanner error={error}/></div>:null}
    <section className="print-controls mx-auto mb-4 max-w-4xl rounded-xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center"><div><p className="text-xs font-bold uppercase tracking-wider text-slate-500">Isolated print run · {run?.runNumber}</p><h1 className="mt-1 text-xl font-black">{template.name} · {labels.length} rendered label{labels.length===1?'':'s'}</h1><p className="mt-1 text-sm text-slate-600">Only the sheet below is sent to the printer. Opening or cancelling the browser dialog does not change print counts.</p></div><Button disabled={decision==='confirmed'||decision==='cancelled'} onClick={()=>{setDialogOpened(true);window.print();}}><Printer className="mr-2 h-4 w-4"/>Open print dialog</Button></div>
      ...
      {decision?<div className={`mt-4 rounded-lg p-3 text-sm font-bold ${decision==='confirmed'?'bg-emerald-50 text-emerald-800':'bg-amber-50 text-amber-800'}`}>{decision==='confirmed'?'Print confirmed and audited. Reprinting will create a new run.':'Run cancelled; no label print counts were changed.'}</div>:null}
    </section>
    <section className="label-sheet mx-auto grid bg-white shadow-xl" style={{width:`${pageWidth}mm`,minHeight:`${pageHeight}mm`,gridTemplateColumns:`repeat(${columns}, ${Number(template.widthMm||70)}mm)`,gridAutoRows:`${Number(template.heightMm||37)}mm`,columnGap:`${Number(template.gapXMm||0)}mm`,rowGap:`${Number(template.gapYMm||0)}mm`,padding:`${Number(template.marginTopMm||0)}mm ${Number(template.marginRightMm||0)}mm ${Number(template.marginBottomMm||0)}mm ${Number(template.marginLeftMm||0)}mm`}}>
      {labels.map((label:any,index:number)=><article key={`${label.id}-${label.copyIndex}-${index}`} className="overflow-hidden border border-black/30 bg-white p-[2mm] text-black [break-inside:avoid]" style={{width:`${Number(template.widthMm||70)}mm`,height:`${Number(template.heightMm||37)}mm`}}><div className="flex h-full items-center gap-[2mm]"><img src={label.qrDataUrl} alt={`QR ${label.labelCode}`} className="h-[24mm] w-[24mm] shrink-0"/><div className="min-w-0 text-[8pt] leading-[1.25]"><p className="truncate text-[10pt] font-black">{label.payload.internalCode||label.payload.sku}</p><p className="line-clamp-2 font-bold">{label.payload.productName}</p><p>{[label.payload.brand,label.payload.dimensions,label.payload.finish].filter(Boolean).join(' · ')}</p><p className="font-semibold">{label.payload.lotNumber||label.payload.displaySample||'Product / shelf'}</p>{label.payload.supplierBatch?<p>Batch {label.payload.supplierBatch}</p>:null}<p className="mt-[1mm] font-mono text-[6.5pt]">{label.labelCode}</p></div></div></article>)}
    </section>
  </main>;
}
