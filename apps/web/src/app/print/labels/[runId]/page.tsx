...
export default function LabelPrintPage({ params }: { params: Promise<{ runId: string }> }) {
  ...
  return <main className="min-h-screen bg-[#f4f1ef] p-4 text-black print:bg-white print:p-0">
    ...
    <section className="label-sheet mx-auto grid bg-white shadow-xl" style={{width:`${pageWidth}mm`,minHeight:`${pageHeight}mm`,gridTemplateColumns:`repeat(${columns}, ${Number(template.widthMm||70)}mm)`,gridAutoRows:`${Number(template.heightMm||37)}mm`,columnGap:`${Number(template.gapXMm||0)}mm`,rowGap:`${Number(template.gapYMm||0)}mm`,padding:`${Number(template.marginTopMm||0)}mm ${Number(template.marginRightMm||0)}mm ${Number(template.marginBottomMm||0)}mm ${Number(template.marginLeftMm||0)}mm`}}>
      {labels.map((label:any,index:number)=><article key={`${label.id}-${label.copyIndex}-${index}`} className="overflow-hidden border border-black/30 bg-white p-[1.6mm] text-black [break-inside:avoid]" style={{width:`${Number(template.widthMm||70)}mm`,height:`${Number(template.heightMm||37)}mm`}}><div className="flex h-full items-center gap-[1.6mm]"><div className="shrink-0"><img src={label.qrDataUrl} alt={`QR ${label.labelCode}`} className="h-[23mm] w-[23mm]"/><p className="mt-[.4mm] text-center font-mono text-[5.6pt] font-bold">{label.labelCode}</p></div><div className="min-w-0 flex-1 text-[7.2pt] leading-[1.18]"><div className="mb-[.8mm] flex items-center justify-between border-b border-black/25 pb-[.6mm]"><p className="truncate text-[9.5pt] font-black">{label.payload.internalCode||label.payload.sku}</p><span className="ml-1 rounded bg-[#a92f28] px-[1.2mm] py-[.3mm] text-[5.5pt] font-black tracking-wide text-white">MP</span></div><p className="line-clamp-2 font-bold">{label.payload.productName}</p><p className="truncate">{[label.payload.brandCode||label.payload.brand,label.payload.dimensions,label.payload.finish].filter(Boolean).join(' · ')}</p><p className="mt-[.6mm] truncate font-black">{label.payload.lotNumber||label.payload.displaySample||'PRODUCT / SHELF'}</p>{label.payload.sourceDocument?<p className="truncate">{label.payload.sourceDocument}{label.payload.locations?.length?` · ${label.payload.locations.join(', ')}`:''}</p>:null}{label.payload.supplierBatch?<p className="truncate">Batch {label.payload.supplierBatch}</p>:null}</div></div></article>)}
    </section>
  </main>;
}
