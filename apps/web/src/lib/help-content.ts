...
export const HELP_GUIDES: HelpGuide[] = [
  ...
  },
  {
    ...
    summary: 'Choose area, piece or box pricing, then apply the retail ladder in the correct order: MRP, NRP discount, optional special discount and optional whole-quote discount.',
    video: '/help/videos/02-tile-pricing-and-quotes.mp4', image: '/help/images/quote-editor-tile-and-sanitaryware.png',
    flow: ['Find / scan item', 'Choose billed UOM', 'MRP', 'NRP discount', 'Special discount', 'Quote discount'],
    examples: ['Training example: MRP is ₹1,000 per box. Enter a 10% NRP discount to reach NRP ₹900. Enter a further 5% special discount on NRP to reach net selling price ₹855. A final 2% whole-quote discount is then allocated proportionally across all lines; it never changes the saved MRP or NRP.'],
    steps: ['Search or scan the internal showroom code or SKU in Quote Studio.', 'For a tile, select Area, Pieces or Boxes. Area needs required area and wastage; pieces are rounded to physical boxes; Boxes uses the physical count directly.', 'Review List rate. It is read-only and comes from Product Master; ask the product-master owner to correct it at source if wrong.', 'Enter the tax-inclusive MRP as an amount for the selected rate basis.', 'Enter the NRP/base discount as % or ₹. Percent is the default and always applies to MRP.', 'If authorised, enter the special discount as % or ₹. It applies to NRP, not back to MRP.', 'After all items are ready, enter the optional whole-quote discount as % or ₹. It is allocated proportionally so PDF, register and Sales Order use one reconciled total.', 'Use Tab to move through fields, / to focus product search, Cmd/Ctrl+S to save draft, Cmd/Ctrl+Enter to validate, and Esc to close a scan panel.', 'Review the billed quantity, physical boxes, MRP, NRP, net selling price, GST and final quote value before saving.'],
    checks: ['MRP is required and must be at least the final tax-inclusive unit payable.', 'NRP discount cannot exceed MRP; special discount cannot exceed NRP; quote discount cannot exceed the line-net total.', 'Area pricing needs positive coverage per box and piece pricing needs pieces per box.', 'List rate is a source reference, not an editable quote field.', 'The Quote Register, PDF and Sales Order conversion must show the same governed total.', 'Orders, reservations and dispatch always use the physical inventory quantity shown.'],
