...
export const HELP_GUIDES: HelpGuide[] = [
  {
    ...
    examples: ['Training example: create design "Calacatta Ivory" once with its images, marble-look material, ivory colour, polished finish and wall/floor usage. Create Size Master "1200 x 600 mm" with calculated area. Then create the inwardable variant that combines that design, size and finish; the system issues its permanent warehouse SKU. Add supplier or old showroom codes as aliases, never by renaming the SKU.'],
  ...
  },
  {
    ...
    summary: 'Use the phone camera while walking the showroom with a customer. Each active MP label resolves to its governed Product Master item before it is added to the lead or intent.',
    flow: ['Customer + lead details', 'Scan showroom item', 'Verify product row', 'Set room / quantity', 'Save intent / create quote'],
    examples: ['Training example: while showing a customer the master bathroom, scan the display label for Calacatta Ivory 1200 x 600. Confirm the returned warehouse/display code and image, set Room / use to "Master bathroom wall", enter the required area and wastage, then scan the next selected item. Create the lead with intent; later scans on the editable Intent page must be saved or submitted before leaving.'],
    steps: ['On mobile, open Leads -> New lead and select the customer, project, source and next action.', 'In Product intent, tap Scan showroom item, then Scan with camera. Allow camera access for the Marble Park HTTPS site when the phone asks.', 'Hold the MP QR inside the frame. A valid active product, lot or display label adds its linked Product Master item; an inactive or unknown label changes nothing.', 'Verify the image, permanent warehouse/display code, design, size, finish and rate shown on the new row. A display asset is non-sellable, but its linked Product Master variant is the commercial item added to intent.', 'Set Room / use and quantity. For area-priced tiles, enter required area and wastage; the system derives physical packs.', 'Scan more items as the customer walks the showroom. Search by code remains available when the camera is blocked or a label is damaged.', 'Choose Create lead + intent. On an existing editable intent, use Scan showroom item, review the new row, then Save draft or Save & submit so the scan persists.', 'From Labels & Scan, a successful scan can also open Catalogue, preload Add to intent, or start Quick quote; always confirm customer and pricing before saving.'],
    checks: ['Sales users need only an authenticated account and their normal lead/intent permission; label printing permission is not required to scan.', 'The camera starts only after a deliberate tap and stops after a successful read.', 'The encoded QR value is never repeated on screen; the printed label code is the fallback.', 'Scanning does not reserve, issue or increase stock.', 'An existing intent must remain editable; submitted, quoted or cancelled intents are frozen for audit.', 'Do not leave the page until the scanned row has been saved or submitted.'],
  ...
  },
  {
    ...
    flow: ['GRN or vendor sample', 'Exact product and lot', 'Display code and position', 'Audited issue / register', 'QR label and lifecycle'],
    examples: ['Training example A: issue 1 piece from the Calacatta Ivory GRN lot to display code WALL-A-014 in zone Tile Gallery, position Bay A. Available stock reduces by 1 and the display remains linked to the original lot. Training example B: register a free vendor sample with its own display code and position; no GRN, lot or stock quantity is created.'],
    steps: ['For anything purchased or intended as saleable stock, post the PO/manual GRN first so the physical receipt has a governed lot.', 'Open Display Assets and choose From inventory. Search the Product Master item, choose the exact available lot and location, enter the quantity, display code, showroom zone and position.', 'Saving posts one display_issue out of that exact lot, creates the non-sellable display asset and prepares its QR label job.', 'Use Vendor sample only for a free/non-stock physical sample. This registers the asset but creates no GRN and no stock movement.', 'Print the display QR from Labels & Scan and attach it to the matching physical sample.', 'Use Inspect and Maintenance for routine lifecycle updates. Remove with a reason; a stock-issued display can return only its complete issued quantity to the original lot.'],
