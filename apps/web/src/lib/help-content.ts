...
export const HELP_GUIDES: HelpGuide[] = [
  ...
  },
  {
    ...
    examples: ['Training example: order 10 boxes of Calacatta Ivory at ₹1,500 per box, 3 pieces per box, with 5% supplier discount and no GST. The PO snapshots ₹500 per piece before discount and ₹475 per piece net pre-tax. The linked GRN asks only for the physical quantity and trace fields; it cannot silently replace that cost. A genuine manual receipt without a PO asks for its supplier rate and UOM before posting the lot.'],
    steps: ['Start on Procurement Overview. Its KPI cards and schedule read the live demand, PO and GRN registers; purchase commitment value includes only POs with a captured commercial value and identifies missing costs instead of treating them as real zero spend.', 'Open Demand for customer shortages or Purchase Orders for planned replenishment. Search, filter and sort the server-paged queue; select only active demand and choose a governed supplier.', 'For a planned PO, search the Product/variant code, enter the physical quantity, supplier rate and Rate UOM. The system normalises a box/set rate through the saved packing factor and keeps both the entered supplier rate and base-unit cost.', 'Record expected date, optional header discount and optional GST. Leave GST blank or enter 0 for a non-GST PO. Review the totals, then create and download the supplier PO PDF.', 'In Receiving, choose the PO and enter only quantities physically delivered; every receipt line starts blank and never defaults to the outstanding balance. The cost is inherited from the PO net pre-tax snapshot and is not editable on the GRN.', 'Capture supplier challan/bill, stock location, damaged pieces, batch and tile shade/caliber/grade. Use Manual GRN only for a real delivery without a prior PO; enter its supplier rate, Rate UOM and reason.', 'If an older open PO says Cost setup needed, an owner/admin must use Complete legacy PO rates before the first receipt. The workflow never deletes the PO and refuses to reprice a line that already has a receipt.', 'Use the searchable selectors with typing, Arrow keys and Enter; Escape closes the list. High-volume PO and GRN registers remain server-paged.', 'After posting, open History and use Create labels on the GRN. It hands the exact accepted lines and lots to the Physical Identity Desk without reposting stock.', 'Review exact lots, Pending Inward allocation and stock reconciliation. The form clears after success.'],
    checks: ['Never add live stock directly to balances.', 'Every new PO line requires a positive supplier rate; GST is optional and blank/0 means without GST.', 'PO-linked GRN cost comes only from the saved PO net pre-tax rate; a receipt cost override is rejected.', 'Manual GRN requires a positive supplier rate and preserves its entered rate/UOM snapshot.', 'Received quantity cannot exceed the remaining PO base quantity.', 'Boxes and loose pieces convert to base pieces with an immutable snapshot.', 'Damaged quantity is isolated from available stock.', 'Every inward retains supplier, PO/manual reason, GRN, variant, lot, location, actor and source links.'],
