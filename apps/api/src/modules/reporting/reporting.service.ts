...
export class ReportingService {
  ...
  async readiness() {
    const [targetCount, quoteLines, invoiceLines, products] = await Promise.all([
      ...
      this.prisma.product.findMany({ select: { id: true, sku: true, name: true, status: true, categoryId: true, brandId: true, finishId: true, unit: true, hsnCode: true, defaultMrpInclusive: true, defaultNrpInclusive: true, priceRateBasis: true, priceUom: true, tileSizeId: true, piecesPerPack: true, category: true } }),
    ]);
    const activeProducts = products.filter((row: any) => row.status === 'active');
    const incompleteProducts = activeProducts.filter((row: any) => !row.sku || !row.name || !row.categoryId || !row.brandId || !row.finishId || !row.unit || !row.hsnCode || ((row.defaultMrpInclusive != null || row.defaultNrpInclusive != null) && (!row.defaultMrpInclusive || !row.defaultNrpInclusive || !row.priceRateBasis || !row.priceUom || n(row.defaultNrpInclusive) > n(row.defaultMrpInclusive))) || (/tile/i.test(row.category) && (!row.tileSizeId || n(row.piecesPerPack) <= 0)));
  ...
  private async masterDataReadiness({ args, definition, range }: any) {
    const products = await this.prisma.product.findMany({ where: { status: 'active' }, orderBy: { updatedAt: 'desc' }, select: { id: true, sku: true, name: true, category: true, categoryId: true, brandId: true, finishId: true, unit: true, hsnCode: true, defaultMrpInclusive: true, defaultNrpInclusive: true, priceRateBasis: true, priceUom: true, tileSizeId: true, piecesPerPack: true, internalCode: true, updatedAt: true } });
    const rows = products.map((row: any) => { const pricingStarted = row.defaultMrpInclusive != null || row.defaultNrpInclusive != null; const pricingInvalid = pricingStarted && (!row.defaultMrpInclusive || !row.defaultNrpInclusive || !row.priceRateBasis || !row.priceUom || n(row.defaultNrpInclusive) > n(row.defaultMrpInclusive)); const missing = [!row.categoryId && 'category master', !row.brandId && 'brand master', !row.finishId && 'finish master', !row.unit && 'unit', !row.hsnCode && 'HSN', pricingInvalid && 'pricing defaults', /tile/i.test(row.category) && !row.tileSizeId && 'tile size', /tile/i.test(row.category) && n(row.piecesPerPack) <= 0 && 'pack quantity'].filter(Boolean); return { id: row.id, sku: row.sku, internalCode: row.internalCode || '', product: row.name, category: row.category, missing: missing.join(', '), exceptionCount: missing.length, updatedAt: row.updatedAt.toISOString(), href: `/dashboard/products?search=${encodeURIComponent(row.sku)}` }; });
    const exceptions = rows.filter((row: any) => row.exceptionCount > 0);
    return { meta: this.meta(definition, { coverage: products.length ? round((products.length - exceptions.length) / products.length * 100, 1) : 100, coverageLabel: 'Active products meeting current governed requirements', warnings: exceptions.length ? ['Correct the authoritative product, tile and size masters; reporting does not provide a bypass entry form.'] : [] }), range: range.labels, filters: args.filters, summary: [this.summary('products', 'Active products', products.length, null, 'number', 'Products in active status.'), this.summary('exceptions', 'Products needing attention', exceptions.length, null, 'number', 'Active products missing one or more current required source fields.'), this.summary('pricing_defaults', 'Pricing default exceptions', rows.filter((row: any) => row.missing.includes('pricing defaults')).length, null, 'number', 'Products with a partially configured or invalid MRP/NRP default pair.'), this.summary('tile_setup', 'Tile setup exceptions', rows.filter((row: any) => row.missing.includes('tile size') || row.missing.includes('pack quantity')).length, null, 'number', 'Tile products missing Size Master or pack quantity.')], trend: [], breakdowns: [{ id: 'exception_type', title: 'Readiness exceptions by source', kind: 'ranking', rows: ['category master', 'brand master', 'finish master', 'unit', 'HSN', 'pricing defaults', 'tile size', 'pack quantity'].map((label) => ({ label, value: rows.filter((row: any) => row.missing.includes(label)).length })).filter((row) => row.value > 0) }], rows: this.paginate(exceptions, args, [{ key: 'sku', label: 'SKU' }, { key: 'internalCode', label: 'Internal code' }, { key: 'product', label: 'Product' }, { key: 'category', label: 'Category' }, { key: 'missing', label: 'Missing source fields' }, { key: 'exceptionCount', label: 'Exceptions', format: 'number' }, { key: 'updatedAt', label: 'Last updated' }], ['exceptionCount', 'updatedAt', 'sku', 'product']) };
  }
  ...
  private async salesProduct({ args, definition, range, user }: any) {
    ...
    const [invoiceLines, priorInvoiceLines, returnLines, priorReturnLines, customers, architects, payments, priorPayments, availability] = await Promise.all([
      ...
      priorInvoiceIds.length ? (this.prisma as any).salesInvoiceLine.findMany({ where: { salesInvoiceId: { in: priorInvoiceIds } }, select: { salesOrderLineId: true, quantity: true, taxableValue: true, grossLineTotal: true } }) : [],
    ...
    for (const line of invoiceLines) {
      ...
      if (base > 0) { weightedBase += base; weightedDiscountValue += Math.max(0, base - n(line.grossLineTotal)); }
    }
    ...
    let priorWeightedBase=0;let priorWeightedDiscount=0;for(const line of priorInvoiceLines){const source:any=orderLineMap.get(line.salesOrderLineId);const sourcePricingQuantity=n((source?.metadata as any)?.snapshot?.pricingQuantity)||n(source?.orderedQuantity);const base=source&&n(source.orderedQuantity)>0?n(source.mrpInclusive)*sourcePricingQuantity*n(line.quantity)/n(source.orderedQuantity):0;if(base>0){priorWeightedBase+=base;priorWeightedDiscount+=Math.max(0,base-n(line.grossLineTotal||line.taxableValue));}}
  ...
  private async auditQuality({ args, definition, range }: any) {
    const current={gte:range.from,lte:range.to};const eventTotal=await this.prisma.auditEvent.count({where:{createdAt:current}});const [events,users,sessions,documents,products,notifications,labels]=await Promise.all([(this.prisma as any).auditEvent.findMany({where:{createdAt:current},orderBy:{createdAt:'desc'},take:5000}),(this.prisma as any).user.findMany({select:{id:true,name:true,role:true,active:true,passwordChangedAt:true,permissionOverrides:true}}),(this.prisma as any).session.findMany({select:{userId:true,expiresAt:true}}),(this.prisma as any).documentJob.findMany({where:{createdAt:current}}),this.prisma.product.findMany({select:{id:true,sku:true,name:true,category:true,brand:true,finish:true,status:true}}),this.prisma.notificationRecipient.count({where:{readAt:null,archivedAt:null,notification:{createdAt:current,OR:[{category:"action",status:"open"},{category:"updates"}]}}}),(this.prisma as any).internalLabelInstance.findMany({where:{createdAt:current}}).catch(()=>[])]);
    const actorIds=Array.from(new Set<string>(events.map((row:any)=>String(row.actorUserId||'')).filter(Boolean)));const actors=actorIds.length?await this.prisma.user.findMany({where:{id:{in:actorIds}},select:{id:true,name:true}}):[];const actorMap=new Map(actors.map((row:any)=>[row.id,row.name]));const actionMix=new Map<string,any>();events.forEach((row:any)=>{const item=actionMix.get(row.action)||{label:row.action,value:0};item.value+=1;actionMix.set(row.action,item)});const incomplete=products.filter((row:any)=>!row.category||!row.brand||!row.name||!row.sku);const failedDocuments=documents.filter((row:any)=>row.status==='failed');const expiredSessions=sessions.filter((row:any)=>row.expiresAt<new Date()).length;const unread=notifications;
