...
def build():
    ...
    for index, guide in enumerate(guides):
        ...
        if not guide.get("image"):
            chapter.append(Table([[""]], colWidths=[178 * mm], rowHeights=[10 * mm]))
        role_label = "ALL AUTHENTICATED USERS" if guide.get("id") == "account-security" else " - ".join(guide["roles"]).upper()
        chapter += [Paragraph(f"{index + 1:02d}  /  {role_label}  /  {guide['duration']}", styles["Eyebrow"]), Paragraph(guide["title"], styles["GuideTitle"]), Paragraph(guide["summary"], styles["BodyMP"]), Spacer(1, 3 * mm), flow_table(guide["flow"]), Spacer(1, 4 * mm)]
        ...
        if guide.get("video"):
            chapter += [Spacer(1, 3 * mm), Paragraph(f"Training video: {Path(guide['video']).name}", styles["BodyMP"])]
