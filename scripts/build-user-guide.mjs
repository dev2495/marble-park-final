...
const logoData = `data:image/png;base64,${fs.readFileSync(path.join(root, 'apps/web/public/brand/marble-park-logo.png')).toString('base64')}`;
...
for (const destination of [publicDir, reportDir]) {
  ...
  fs.writeFileSync(path.join(destination, 'guide-data.json'), JSON.stringify({ generatedAt, guides }, null, 2));
...
fs.writeFileSync(path.join(publicDir, 'client-handover.html'), clientHandoverHtml);

console.log(JSON.stringify({ ok: true, guides: guides.length, videos: uniqueVideos.length, publicHtml: path.join(publicDir, 'user-guide.html'), clientHandoverHtml: path.join(publicDir, 'client-handover.html'), reportHtml: path.join(reportDir, 'guide.html') }, null, 2));
