...
async function main() {
  ...
  const printRun = (await gql('mutation($input:InternalLabelPrintRunInput!){prepareInternalLabelPrintRun(input:$input)}', { input: { labelJobId: labelJob.id, templateCode: 'thermal_4x2', labelSize: '4x2_in', labelIds: [labelJob.instances[0].id], copies: 1, reason: 'Clone display print acceptance' } }, token)).prepareInternalLabelPrintRun;
